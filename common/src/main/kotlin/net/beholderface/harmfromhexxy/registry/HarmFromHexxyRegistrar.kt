package net.beholderface.harmfromhexxy.registry
...
typealias RegistrarEntry<T> = HarmFromHexxyRegistrar<T>.Entry<out T>

abstract class HarmFromHexxyRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(HarmFromHexxy.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HarmFromHexxyRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
