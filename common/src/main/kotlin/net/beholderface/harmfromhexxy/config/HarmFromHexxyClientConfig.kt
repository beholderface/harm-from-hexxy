package net.beholderface.harmfromhexxy.config
...
object HarmFromHexxyClientConfig {
    @JvmStatic
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HarmFromHexxyServerConfig.holder.config = HarmFromHexxyServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HarmFromHexxyServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = HarmFromHexxy.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HarmFromHexxyServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HarmFromHexxyServerConfig.ServerConfig()
    }
